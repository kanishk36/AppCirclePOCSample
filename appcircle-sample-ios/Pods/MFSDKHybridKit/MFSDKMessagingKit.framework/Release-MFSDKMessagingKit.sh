...
MF_BUILD_VERSION=$((MF_BUILD_VERSION+1))
MF_COMPLETEURL=${MF_BASEURL2}${MF_BASE_VERSION}.${MF_BUILD_VERSION}${MF_FILEZIP}
...
echo "MF_BASEURL2='${MF_BASEURL2}'" >> build.properties
echo "MF_FILEZIP='${MF_FILEZIP}'" >> build.properties
echo "MF_COMPLETEURL=${MF_BASEURL2}${MF_BASE_VERSION}.${MF_BUILD_VERSION}${MF_FILEZIP}" >> build.properties
...
#curl -udocs:AP68Gi3oiUeEjRjvi1FcWzNt9HV -T "${COMPILER_OUTPUT}/build/Release-universal/MFSDKHybridKit.zip" "${MF_COMPLETEURL}"

curl -u "build_repo_in@active.ai:GhRV%3NL2y" -T "${COMPILER_OUTPUT}/build/Release-universal/MFSDKHybridKit.zip" "${MF_COMPLETEURL}"
...
pod spec lint MFSDKHybridKit.podspec --verbose --allow-warnings
echoTask "POD VALIDATION DONE"

pod trunk push MFSDKHybridKit.podspec --verbose --allow-warnings
